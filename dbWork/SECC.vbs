...
CurrentDb.Execute "DELETE FROM SECC", dbFailOnError
...
'filePath = "students.csv"
    ...
    If Not (rs.EOF And rs.BOF) Then
        ...
        Do Until rs.EOF = True 'Loop through all the records one at a time
            If Not IsNull(rs!StuStatusTrans) Then
                test = rs!StuStatusTrans
            Else
                test = ""
            End If
            'If test1 Like rs!DistID And rs!StuStatus = "Active" Then 'checking to see if student is in system.
            If test1 Like rs!DistID And (rs!StuStatus = "Active" Or (rs!StuStatus = "Intake Inprocess" And InStr(1, test, "Transfer") <> 0)) Then 'checking to see if student is in system.
                err = 1
            End If
             rs.MoveNext
        Loop
