...
CurrentDb.Execute "DELETE FROM SECCexit", dbFailOnError
...
'filePath = "students.csv"
    ...
    If Not (rs.EOF And rs.BOF) Then
        ...
        Do Until rs.EOF = True 'Loop through all the records one at a time
            ...
            'If test1 Like rs!DistID And rs!StuStatus = "Active" Then 'checking to see if student is in system.
            If Not IsNull(rs!IEPupdateDate) Then
            Dim IEPupdateDate1 As Date
            IEPupdateDate1 = CDate(rs!IEPupdateDate)
                If test1 Like rs!DistID And IEPupdateDate1 < CDate("06/30/2017") And IEPupdateDate1 > CDate("07/01/2016") Then
                    err = 1
                End If
            End If
             rs.MoveNext
        ...
        Set sped = CurrentDb.OpenRecordset("SELECT * FROM stuData WHERE DistID = " & test1)
            If Not (sped.EOF And sped.BOF) Then
                ...
                Do Until sped.EOF = True 'Loop through all the records one at a time
                        ...
                        SECCEligDtTxt = sped!LastReEvalDate
                        ...
                        If (Not sped!inactiveReason Like "") Then
                            SECCRsnExtCd = sped!inactiveReason '84
                            SECCRsnExtCd = Left(SECCRsnExtCd, 2)
                        End If
            ...
            stringSQL = "INSERT INTO SECCexit " _
            ...
            & "[SECCRecTypCd],[SECCSuppSvc1],[SECCSuppSvc2],[SECCSuppSvc3],[SECCSuppSvc4],[SECCSuppSvc5],[SECCSuppSvc6],[SECCResdDistInstID]," _
            & "[SECCPrimLangCd],[SECCEarlyEntryFg], " _
            & "[ResdDistInstID],[ResdSchlInstID],[SECCPrimDsbltyCd],[SECCSecDsblty1Cd],[SECCSecDsblty2Cd],[SECCRltdSvc1],[SECCRltdSvc2],[SECCRltdSvc3]," _
            & "[SECCRltdSvc4],[SECCRltdSvc5],[SECCRltdSvc6],[SECCFedPlCd],[SECCAgySrvCd],[SECCEligDtTxt],[SECCLstIEPDtTxt],[SECCSpEdExitDtTxt]," _
            & "[SECCRsnExtCd],[SECCEnrlTyp],[SECCSecFedPlCd])" _
            & "VALUES (" _
            & "'" & ChkDigitStdntID & "','" & DistStdntID & "','" & AttndDistInstID & "','" & AttndSchlInstID & "','" & LglLNm & "','" & LglFNm & "','" & LglMNm & "','" & GnrtnCd & "'," _
            & "'" & PrfrdLNm & "','" & PrfrdFNm & "','" & PrfrdMNm & "','" & BirthDtTxt & "','" & GndrCd & "','" & HispEthnicFg & "','" & AmerIndianAlsknNtvRaceFg & "','" & AsianRaceFg & "'," _
            & "'" & BlackRaceFg & "','" & WhiteRaceFg & "','" & PacIslndrRaceFg & "','" & LangOrgnCd & "','" & SSN & "','" & EnrlGrdCd & "','" & Addr & "','" & City & "'," _
            ...
            & "'" & ResdDistInstID & "','" & ResdSchlInstID & "','" & SECCPrimDsbltyCd & "','" & SECCSecDsblty1Cd & "','" & SECCSecDsblty2Cd & "','" & SECCRltdSvc1 & "','" & SECCRltdSvc2 & "','" & SECCRltdSvc3 & "'," _
            & "'" & SECCRltdSvc4 & "','" & SECCRltdSvc5 & "','" & SECCRltdSvc6 & "','" & SECCFedPlCd & "','" & SECCAgySrvCd & "','" & SECCEligDtTxt & "','" & SECCLstIEPDtTxt & "','" & SECCSpEdExitDtTxt & "'," _
            & "'" & SECCRsnExtCd & "','" & SECCEnrlTyp & "','" & SECCSecFedPlCd & "');  "
